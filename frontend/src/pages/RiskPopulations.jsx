...
export default function RiskPopulations({ embedded = false }) {
  ...
  if (!data && busy) return <Spinner label="Scoring high-risk populations…" />;
  if (!data && err) {
    return (
      <Card className="p-4">
  ...
  if (!data) return <Spinner label="Scoring high-risk populations…" />;

  const findings = data.findings || data.segments || [];
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <input
          ...
          className="flex-1 rounded-lg bg-slate-900 border border-slate-700 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500"
        />
        <Button variant="primary" disabled={busy} onClick={runPredict}>
          {busy ? 'Scoring…' : 'Predict segments'}
        </Button>
      ...
      <div className="flex flex-wrap gap-2 text-[11px] text-slate-500">
        ...
        <span>· n={data.n_training_rows}</span>
        <span>· positives={data.n_positive}</span>
        <span>· baseline={data.baseline_risk}</span>
      </div>
      ...
      {findings.length === 0 ? (
        <Card className="p-4 text-sm text-slate-400">
          {data.needs_demo_seed
            ...
            : 'No elevated segments for this pair at the current confidence threshold.'}
        </Card>
      ) : (
        <div className="space-y-3">
          {findings.map((s) => (
            <Card key={s.segment_id || s.label} className="p-4 border-amber-700/25">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-2 text-sm text-slate-300 leading-relaxed">{s.actionable_insight}</p>
                  {(s.top_contributing_factors || []).length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {s.top_contributing_factors.map((f) => (
                        ...
                        >
                          {f.factor} {f.shap_value > 0 ? '+' : ''}{f.shap_value}
                        </span>
                      ))}
                    </div>
                  )}
